...
def preprocessamento_fertilizantes(file_path: str) -> pd.DataFrame:
    ...
    # Verificar e remover as colunas 'Area Code' e 'Area Code (M49)' se existirem
    cols_to_drop_optional = ['Area Code', 'Area Code (M49)']
    df.drop(columns=[col for col in cols_to_drop_optional if col in df.columns], inplace=True)
