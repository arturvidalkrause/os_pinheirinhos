...
def preprocessamento_fertilizantes(path):
    # Lendo o arquivo
    try:
        df = pd.read_csv(os.path.join(path, "Fertilizantes_por_nutrientes.csv"), encoding='utf-8')
    except UnicodeDecodeError:
        # Se ocorrer um erro de decodificação, tenta com uma codificação diferente
        df = pd.read_csv(os.path.join(path, "Fertilizantes_por_nutrientes.csv"), encoding='ISO-8859-1')
    ...
    df_melted = df_total.melt(id_vars=['Area'], var_name='ano', value_name='uso_total_de_fertilizantes(t)')
    ...
    def preencher_anos_faltantes(df):
        ...
        df_completo = pd.merge(df_todos_anos, df, on=['country_code', 'ano'], how='left').replace(0, np.nan)
