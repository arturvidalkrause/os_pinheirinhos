...
def preprocessamento_producao(file_path: str) -> pd.DataFrame:
    ...
    # Lendo o arquivo
	try:
		df = pd.read_csv(os.path.join(file_path, "Produção agropecuaria.csv"), encoding='utf-8')
	except UnicodeDecodeError:
        # Se ocorrer um erro de decodificação, tenta com uma codificação diferente
		df = pd.read_csv(os.path.join(file_path, "Produção agropecuaria.csv"), encoding='ISO-8859-1')
